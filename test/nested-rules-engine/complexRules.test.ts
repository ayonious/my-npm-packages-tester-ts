...
describe('Nested Rules Engine: Complex Rules', () => {
  it('should handle deeply nested rules', () => {
    ...
    const adminHighResult = executeEngine(adminHighInput, functions, rules) as EngineResult;
    ...
    const adminMediumResult = executeEngine(adminMediumInput, functions, rules) as EngineResult;
    ...
    const adminLowResult = executeEngine(adminLowInput, functions, rules) as EngineResult;
    ...
    const managerHighResult = executeEngine(managerHighInput, functions, rules) as EngineResult;
    ...
    const userResult = executeEngine(userInput, functions, rules) as EngineResult;
    ...
    const unauthenticatedResult = executeEngine(unauthenticatedInput, functions, rules) as EngineResult;
  ...
  it('should handle rules with multiple conditions', () => {
    ...
    const functions = {
      ...
      has_sufficient_inventory: ({ order, inventory }) => {
        return order.items.every(item => 
          inventory[item.id] && inventory[item.id] >= item.quantity
        );
      },
    ...
    const validOrderResult = executeEngine(validOrderInput, functions, rules) as EngineResult;
    ...
    const pendingPaymentInput = {
      order: {
        ...
        items: [
          { id: 'prod-001', name: 'Widget', quantity: 1, price: 10.99 }
        ],
        totalAmount: 10.99,
    ...
    const pendingPaymentResult = executeEngine(pendingPaymentInput, functions, rules) as EngineResult;
    ...
    const insufficientInventoryInput = {
      order: {
        ...
        items: [
          { id: 'prod-001', name: 'Widget', quantity: 20, price: 10.99 }
        ],
        totalAmount: 219.80,
        paymentStatus: 'approved'
      },
    ...
    const insufficientInventoryResult = executeEngine(insufficientInventoryInput, functions, rules) as EngineResult;
    ...
    const invalidOrderResult = executeEngine(invalidOrderInput, functions, rules) as EngineResult;
